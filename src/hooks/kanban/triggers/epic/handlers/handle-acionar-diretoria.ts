...
export async function handleAcionarDiretoria(
  user: UserDTO,
  ...
  if (isQualityMember) {
    const toInvalidaTransition = transitions.find((transition: TransitionProps) => transition.to.id === JiraStatusesId.AGUARDANDO_APROVACAO);
